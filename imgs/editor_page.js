...
//ds,bhjfsakdaskdksadbasbj

/*useCodeEditor((editor) => {
    let isPasting = false;
    let isHighlighted = false;
    let highlightedTxt = { start: 0, end: 0 };
        ...
        event.changes.forEach(change => {
            ...
            if (isDelete || isPaste) {
                if (isDelete) {
                    ...
                    if (isHighlighted) {
                        const firstline = lines[highlightedTxt.start];
                        DeleteHighhlighted(highlightedTxt.start, highlightedTxt.end, lines.length, firstline);
                        isHighlighted = false;
                    } else {
                        // Delete lines in reverse order (from last to first)
                        for (let i = endLineNumber - 1; i >= startLineNumber - 1; i--) {
                            const modification = JSON.stringify({
                                content: lines[i],                   // The content of the line being deleted
                                row: i,                             // Convert to 0-based index
                                action: 'delete',                    // Action type (delete)
                                linesLength: lines.length           // Total number of lines in the editor
                            });

                            // Log the modification for debugging
                            console.log('Delete Modification:', modification);

                            // Call saveInput with the modification
                            saveInput(modification);
                        }

                        // Update the first line after deletion
                        const firstline = lines[startLineNumber - 1]; // Get the first line after deletion
                        const modification = JSON.stringify({
                            content: firstline,                      // The content of the first line
                            row: startLineNumber - 1,               // Convert to 0-based index
                            action: 'update',                       // Action type (update)
                            linesLength: lines.length               // Total number of lines in the editor
                        });
                        ...
                        // Log the modification for debugging
                        console.log('Update First Line Modification:', modification);
                    ...
                    // Process the last line
                    if (isLastLineUpdate && modifiedLines.length > 0) {
                        // Update the existing line with the last line of the pasted content
                        const modification = JSON.stringify({
                            content: modifiedLines[modifiedLines.length - 1], // Last line of the pasted content
                            row: startLineNumber + modifiedLines.length - 1,  // Convert to 0-based index
                            action: 'update',                                 // Update the existing line
    ...
    editor.onKeyDown((event) => {
        ...
        if (isHighlighted) {
            ...
            if (event.key === 'Backspace' || event.keyCode === 8) {
                const firstline = lines[highlightedTxt.start];
                DeleteHighhlighted(highlightedTxt.start, highlightedTxt.end, linesLength, firstline);
            }
            isHighlighted = false;
        } else {
    ...
    editor.onDidPaste(() => {
        isPasting = true;
        const row = editor.getPosition().lineNumber - 1;
        const text = editor.getModel().getLineContent(row + 1);
        const linesLength = editor.getValue().split('\n').length - 1;
        const modification = JSON.stringify({ content: text, row, action: 'update', linesLength });
        saveInput(modification);
        setTimeout(() => {
            isPasting = false;
